...
/*
For: static struct page *page_for_addr (const void *address) 
Insperation from (also noted below):
  ...
  if (address < PHYS_BASE) 
    {
	  ...
	  /*
	  Here we stored the stack pointer from intr_handler and the current thread, and
	  since the page can fault up to 32 bytes we added 32 to the address to account for that.
	  Also needed to make sure that the address is within the STACK_MAX, if both
	  of those are met then a page is allocated.
	  Insperation from:
	  https://github.com/LucasSeveryn/pintOS/blob/master/src/vm/page.c
	  http://courses.cs.vt.edu/~cs3204/spring2009/pintos-vt-local/Project3Session.pdf  
	  */
	  ...
	  //boundary 
	  int *boundary = PHYS_BASE - STACK_MAX;

	  //checks to see if addr is within 32 bytes of stack pointer
	  //and also that it is within the STACK_MAX with respect to PHYS_BASE
      //if both are met, a new page is allocated with address
	  if((address + 32 > esp) && (address > boundary)) return page_allocate(address, false);
...
/*
For: bool page_out (struct page *p)
Inspiration from:
...
page_out (struct page *p) 
{
  ...
  /*
  Per comment block directly below the 2 ASSERT statments, we are
  marking the page no present in the page table.
  */
  ...
  /*
  Checking to see if the page has been modified by checking
  ...
  */
  ...
  //page file
  struct file *pfile = p->file;

  //kernel virtual base address
  const void *kpage = p->frame->base;

  //size to write
  off_t size = p->file_bytes;

  //file offset
  off_t file_offset = p->file_offset;

  /*
  If the page has a file, and is dirty we write to it
  from kpage to the file
  */
  if(pfile && dirty) file_write_at(pfile, kpage, size, file_offset);
		
  /*
  Swap out page and keep bool value, and eventually return it,
  after the frame is set to NULL
  */
  ok = swap_out(p);
  p->frame = NULL;
