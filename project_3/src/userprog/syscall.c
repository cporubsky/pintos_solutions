...
/*
For: static void unmap (struct mapping *m)
Here we are essentially undoing what was done in static int
sys_mmap (int handle, void *addr). In sys_mmap, m->page_cnt
was initialized at zero, so we knew we had to loop through.
We also noticed that an address was set to m->base, so as we 
looped through we advanced addr. Also in sys_mmap,
&m->elem was added to a list, so here we need to remove it, as
well as close the file that was opened in sys_mmap and free 'm'.
Insperation from:
...
unmap (struct mapping *m) 
{
  ...
  //page count
  size_t page_count = m->page_cnt;

  //address
  uint8_t *addr = m->base;

  //file to close
  struct file *mfile  = m->file;
  
  //loop through to deallocate page by address
...
sys_munmap (int mapping) 
{
  ...
  /*
  Here we used the lookup_mapping to find the mapping fd handle 
  so we could send it to unmap to continue the unmapping process
  */

  //get the hand from mapping
  struct mapping *handle = lookup_mapping(mapping);

  //if it errors then exit with -1
  if(!handle) return -1;
